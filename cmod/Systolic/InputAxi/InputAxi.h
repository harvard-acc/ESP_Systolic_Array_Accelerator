 ...
 public: 
  ...
  void MasterRead() {
    ...
    while(1) {
      ...
      //   matrix size = N*(M_1+1), num_words = matrix_size / 8
      num_words = (N*(M_1+1)) >> 3;
      ...
      while (num_words > 0) {
        ...
        rd_addr_pld.addr = base_addr + burst_count*8*256;
        if (num_words > 256) {
        ...
        for (int i = 0; i <= rd_addr_pld.len; i++) {
          ...
          for (int j = 0; j < N; j++) {
            if (j < 8) {
              //NVUINT8 tmp = nvhls::get_slc<8>(rd_data_pld.data, 8*j);
              //req_reg.data[j].set_slc<8>(0, tmp);
              req_reg.data[j] = nvhls::get_slc<8>(rd_data_pld.data, 8*j);
              //cout << dec << j << "\t" << req_reg.data[j] << endl;
              req_reg.addr[j] = j + 8*i + burst_count*8*256;
	            req_reg.valids[j] = 1;
	          }
	          else {
              req_reg.data[j] = 0; 
  ...
  void MasterWriteReq() {
    ...
    while(1) {
      ...
      //req_reg.opcode = LOAD;  // Read from Memory.h
  ...
  void MasterWrite() {
    ...
    while(1) {
      ...
      while (num_words > 0) {
        ...
        wr_addr_pld.addr = base_addr + burst_count*8*256;
        if (num_words > 256) {
          wr_addr_pld.len = 255;
	        num_words -= 256;
        }
        ...
        w_req_trig_tmp.addr = 256*8*burst_count; // base addr of read req
